      ...
      <div class="row">
        ...
        <div class="column column-33">
          ...
          <div class="item-info__data">
            ...
            {{
              Math.floor(data.duration / (60 * 100)) -
              Math.floor(data.duration / (3600 * 100)) * 60
            }}м
          </div>
        </div>
        <div class="column column-33 centred-column">
          <div class="item-info__name" v-if="data.stops.length === 0">
            Без пересадок
          </div>
          <div class="item-info__name" v-if="data.stops.length > 0">
            {{ data.stops.length }} {{ stopsWord(data.stops.length) }}
          </div>
          <div class="item-info__data" v-if="data.stops.length > 0">
            {{ data.stops.join(", ") }}
          </div>
        </div>
      </div>
